...
public class SilenceOrb extends Fireball {
	private int range = 9;
	private int age = 0;
		...
		else
		{
			...
			if(mob instanceof LivingEntity livingEntity) {
				for(int i = 0; i < 3; i++) {
					...
					this.level().addParticle(ParticleTypes.ENCHANT, mob.getRandomX(1.0), mob.getRandomY() + 0.5, mob.getRandomZ(1.0), xa, ya, za);
				}
			}
		}
