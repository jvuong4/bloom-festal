...
public class DualStanceCompat implements BFCompat {
	...
	public static @Nullable Entity findTargetByUuid(UUID uuid, ServerLevel level) {
		return level.getEntity(uuid);
	}

	//this should teleport `user`'s pair up partner to `orb`, give or take a few blocks.
	public static void teleportLinkedPlayer(Player user, RewarpOrb orb, ServerLevel level, Vec3 RewarpOrbOldPos)
	{
		for (ItemStack itemStack : user.getInventory()) {
			...
			if (itemStack.has(DualStanceComponents.LINKED_MOB)) {
				...
				UUID partnerUUID = itemStack.get(DualStanceComponents.LINKED_MOB).id();
				if(partnerUUID == null)
					return;
				Entity partner = findTargetByUuid(partnerUUID, level);
				//
				if (partner != null) {
